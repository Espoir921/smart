...
#include "ui_monthlyincomedetailpage.h"
#include <QLabel>
#include <QHeaderView>
#include <QBarSet>
#include <QChart>
...
MonthlyIncomeDetailPage::MonthlyIncomeDetailPage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MonthlyIncomeDetailPage) {
    ui->setupUi(this);
    initUI();
    loadSummaryData();
}

MonthlyIncomeDetailPage::~MonthlyIncomeDetailPage() {
    delete ui;
}

void MonthlyIncomeDetailPage::initUI() {
    QVBoxLayout *mainLayout = new QVBoxLayout(this);
    mainLayout->setContentsMargins(10, 10, 10, 10);
    mainLayout->setSpacing(15);

    QLabel *titleLabel = new QLabel("月收入明细表", this);
...
void MonthlyIncomeDetailPage::loadSummaryData() {
    ...
    if (db.open("try.db")) {
        ...
        QDate endOfMonth = QDate(currentDate.year(), currentDate.month(), currentDate.daysInMonth());

        for (int i = 0; i < incomeTypes.size(); ++i) {
               QSqlQuery query(db.db);
               // 修正：统一使用命名占位符
               query.prepare("SELECT SUM(amount), COUNT(*) FROM income WHERE type=:type AND date BETWEEN :start AND :end");
               query.bindValue(":type", incomeTypes[i]);
               query.bindValue(":start", startOfMonth);
               query.bindValue(":end", endOfMonth);
